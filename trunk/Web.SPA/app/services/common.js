...
commonModule.controller('CustomDialogController', ['$scope', 'dialog', 'model', '$compile', function ($scope, dialog, model, $compile) {
    $scope.title = model.title;
    $scope.buttons = model.buttons;
    $scope.dialog = dialog;
    if (model.init) {
...
commonModule.provider("customDialog", function () {
    var defaults = {
        resolve: {}
    };
...
commonModule.factory('utils', ['$http', 'customDialog', 'consts', function ($http, customDialog, consts) {
    var utils = {
    ...
    function createGrid(settings) {
        ...
        $scope.getPagedDataAsync = function (fromFilter) {
            var data = {
                pageSize: $scope.gridPagingOptions.pageSize,
                page: $scope.gridPagingOptions.currentPage
            };

            if ($scope.gridFilter) {
                $.each($scope.gridFilter, function (key, value) {
                    data[key] = value;
                });
            }
            ...
            $http.post(settings.pageUrl, data)
                .success(function (result) {
                    ...
                    if (result.total) {
                        ...
                        $scope.gridTotalItems = result.total;
                        $scope.setPagingData(result.data);
                    }
                    else {
        ...
        $scope.$watch('gridFilter', function (newVal, oldVal) {
            if (newVal !== oldVal) {
                $scope.getPagedDataAsync(true);
            }
        }, true);
        ...
        for (var i = 0, len = settings.columnDefs.length; i < len; i++) {
            ...
            if (col.comment) {
                ...
                if (!$scope.gridShowComment) {
                    $scope.gridShowComment = function (row) {
                        messageBox("Комментарий", row.entity.comment);
                    }
                }
            }
            ...
            else if (col.blankLink) {
                def.cellTemplate = '<a href="' + col.blankLink.href + '" class="btn btn-small btn-link" target="_blank"><i class="glyphicon glyphicon-' + col.blankLink.icon + '" /></a>';
            }
            else if (col.cellTemplate) {
                def.cellTemplate = col.cellTemplate;
            }

            if (col.cellFilter) {
                def.cellFilter = col.cellFilter;
            }
        ...
        $scope.gridOptions = {
            ...
            enablePaging: true,
            showFooter: true,
            totalServerItems: 'gridTotalItems',
...
commonModule.factory('customGrid', ['$http', 'consts', 'dialogs', function ($http, consts, dialogs) {
    var builder = {
        build: build
    };
    return builder;
